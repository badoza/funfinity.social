...
import { MapPin, Calendar, Users, PlusCircle, Home, ShieldCheck, Search, Camera, Heart, Star, Navigation, Coffee, Sparkles, X, Image as ImageIcon, Video, AlertCircle, CheckCircle2, ChevronRight, LogOut, LayoutDashboard, Instagram, Youtube } from 'lucide-react';
...
export default function FunfinityApp() {
  ...
  // Social URL Formatter (Converts normal Insta/YT links to playable iframes)
  const getEmbedUrl = (url) => {
    if (!url) return '';
    let embedUrl = url;
    if (embedUrl.includes("instagram.com")) {
      embedUrl = embedUrl.split("?")[0]; // Remove query params
      ...
      <section className="py-24 px-4 bg-white border-t border-[#F3E8D8]">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12">
            ...
            {isAdmin && (
               <button onClick={() => setCurrentView('admin')} className="mt-4 md:mt-0 text-sm font-bold text-[#D48847] bg-[#D48847]/10 px-5 py-2.5 rounded-full hover:bg-[#D48847]/20 transition-colors flex items-center gap-2">
    ...
    <div className="w-full bg-[#FDFBF7] min-h-screen pt-8 pb-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
          <div>
            <h2 className="text-4xl font-extrabold tracking-tight text-[#4A3B32] mb-2">Upcoming Events</h2>
            <p className="text-[#4A3B32]/70 font-medium text-lg">Book your spot before they fill up.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-32 bg-white rounded-[2.5rem] border border-[#F3E8D8] shadow-sm">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredEvents.map(event => {
              ...
              return (
                <div key={event.id} className="bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-[#F3E8D8] flex flex-col group cursor-pointer">
                  <div className="h-56 overflow-hidden relative shrink-0 m-2 rounded-[1.5rem]">
                    ...
                    <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-4 py-1.5 rounded-full text-sm font-bold text-[#4A3B32] shadow-sm">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <span className="bg-[#FDFBF7] text-[#4A3B32]/70 text-[10px] px-2 py-1 rounded-md font-bold uppercase tracking-wider">{event.vibe}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-gray-100 mt-auto">
                      ...
                      {isHost ? (
                        ...
                        <button onClick={() => handleRSVP(event.id)} className="bg-[#D48847] text-white px-6 py-2.5 rounded-full font-bold text-sm hover:bg-[#b87439] transition-colors shadow-md transform active:scale-95">
                          RSVP
                        </button>
                      )}
                    </div>
        ...
        <form onSubmit={handleCreateEvent} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
               ...
               <input type="number" placeholder="Leave empty for Free" value={newEvent.price} onChange={e => setNewEvent({...newEvent, price: e.target.value})} className="w-full px-5 py-4 rounded-2xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-[#4A3B32] outline-none transition-all text-gray-900 font-medium" />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div>
      ...
      <div className="bg-[#FDFBF7] p-8 rounded-[2rem] border border-[#F3E8D8]">
        ...
        <form onSubmit={handleAddMemory} className="space-y-5">
          {newMemory.type === 'upload' && (
             <div>
                <label className="block text-sm font-bold text-[#4A3B32] mb-2">Select File (Max 800KB)</label>
